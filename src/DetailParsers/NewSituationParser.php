...
class NewSituationParser implements ParserInterface
{
    ...
    public function parse($coda8_line)
    {
        ...
        $coda8->account                   = substr(
            $coda8_line,
            4,
            37
        ); // don't parse info as it is already present in coda1-line (OriginalSituation)
        $coda8->statement_sequence_number = trim(substr($coda8_line, 1, 3));
        $coda8->date                      = "20" . substr($coda8_line, 61, 2) . "-" . substr(
                $coda8_line,
                59,
                2
            ) . "-" . substr($coda8_line, 57, 2);
